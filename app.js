(() => {
  ...
  const defaultSettings = {
    brand: "SULTAN",
    gallery: "ART GALLERY",
    eyebrow: "Digital Artist · Character Creator",
    statement: "MY ART.\nMY CHARACTERS.\nMY WORLD.",
    font: "default",
    fontSize: 100,
    backgroundType: "default",
    backgroundValue: "",
    siteTitle: "Sultan Art Gallery",
    siteDescription: "My art. My characters. My world.",
    socialImage: "",
    homeVideo: "",
  };
  const coreSections = [
    {
      ...
      slug: "my-drawings",
      name: "MY DRAWINGS",
      description: "Original artwork drawn by me.",
      view: "my-drawing",
      tone: "violet",
      order: 0,
    },
    {
      ...
      slug: "ai-art",
      name: "AI ART",
      description: "AI-assisted artwork and character visuals.",
      view: "ai-art",
      tone: "indigo",
      order: 1,
    },
    {
      ...
      slug: "nekorin",
      name: "NEKORIN",
      description: "A focused artwork collection.",
      view: "Nekorin",
      tone: "purple",
      order: 2,
    },
    {
      ...
      slug: "videos",
      name: "VIDEOS",
      description: "Short-form visual work.",
      view: "videos",
      tone: "video",
      order: 3,
    },
  ...
  const allSections = () => [
    ...
    ...mediaStore
      ...
      .map((section, index) => ({
        ...section,
        view: section.slug,
        tone: section.tone || "violet",
        order: section.order ?? index + 4,
      })),
  ];
  function applySiteSettings() {
    const value = { ...defaultSettings, ...mediaStore.settings() };
    document.querySelectorAll(".brand").forEach((brand) => {
      brand.innerHTML = `<span>${escapeHtml(value.brand)}</span> ${escapeHtml(value.gallery)}`;
    });
    const heroEyebrow = document.querySelector(".hero-copy .eyebrow");
    if (heroEyebrow) heroEyebrow.textContent = value.eyebrow;
    const statement = document.querySelector(".statement");
    if (statement)
      statement.innerHTML = escapeHtml(value.statement).replace(/\n/g, "<br>");
    if (value.font !== "default")
      document.documentElement.style.setProperty(
        "--display",
        `"${value.font}", sans-serif`,
      );
    else document.documentElement.style.removeProperty("--display");
    document.documentElement.style.setProperty(
      "--heading-scale",
      String((Number(value.fontSize) || 100) / 100),
    );
    document.body.dataset.backgroundType = value.backgroundType;
    if (value.backgroundType === "color")
      document.body.style.background = value.backgroundValue || "var(--bg)";
    else if (value.backgroundType === "gradient")
      document.body.style.background =
        value.backgroundValue || "linear-gradient(120deg,#07070d,#180b2b)";
    else if (value.backgroundType === "image" && value.backgroundValue)
      document.body.style.background = `linear-gradient(rgba(7,7,13,.7),rgba(7,7,13,.7)),url("${value.backgroundValue.replace(/["'()]/g, "")}") center/cover fixed`;
  ...
  function preserveOwnerNavigation() {
    ...
    document.querySelectorAll("a[href]").forEach((link) => {
      ...
      if (!raw || raw.startsWith("#") || /^(https?:|mailto:|tel:)/i.test(raw))
        return;
...
if (featured) {
  const renderFeatured = () => {
    ...
    if (settings.homeVideo) {
      featured.innerHTML = `
        <div class="featured-frame">
          <video
            src="${escapeHtml(settings.homeVideo)}"
            controls
            playsinline
            preload="metadata"
          ></video>
    ...
    const allImages = mediaStore
      .all()
      .filter((item) => item.type === "image");

    const art =
      allImages.find(
        (item) => item.source !== "repository" && item.featured,
      ) ||
      allImages.find((item) => item.featured) ||
      allImages[0];
    ...
    if (art) {
      featured.innerHTML = `
        <div class="featured-frame">
          <img
            src="${mediaStore.viewer(art)}"
            alt="${escapeHtml(art.title || "Featured artwork")}"
          >
        </div>
      `;
    }
  ...
  const settings = {
    "my-drawings": [
      "MY DRAWINGS",
      "Personal archive",
      "Original artwork drawn by me.",
    ],
    "ai-art": [
      "AI ART",
      "AI artwork archive",
      "AI-assisted artwork and character visuals.",
    ],
    nekorin: [
      "NEKORIN",
      "Focused collection",
      "Nekorin artwork across both gallery categories.",
    ],
    videos: ["VIDEOS", "Moving image archive", "Short-form visual work."],
  };
  let customCurrent = mediaStore
    .sections()
    .find((section) => section.slug === current);
  if (customCurrent)
    settings[current] = [
      customCurrent.name,
      "Public collection",
  ...
  if (!settings[current])
    settings[current] = [
      current.replace(/-/g, " ").toUpperCase(),
      "Public collection",
      "Selected artwork collection.",
    ];
  const navKey =
    {
  ...
  function renderGallery() {
    ...
    if (customCurrent) {
      ...
      document.querySelector("#gallery-title").textContent =
        settings[current][0];
      document.querySelector("#gallery-kicker").textContent =
        settings[current][1];
      document.querySelector("#gallery-intro").textContent =
        settings[current][2];
      document
    ...
    visible.forEach((item, index) => {
      ...
      mediaButton.className = "artwork-open";
      mediaButton.type = "button";
      mediaButton.setAttribute(
        "aria-label",
        `Open ${item.title || (item.type === "video" ? "video" : "artwork")}`,
      );
      ...
      if (
        mediaStore.ownerRequested() &&
        mediaStore.isOwner() &&
        item.source !== "repository"
      )
        card.appendChild(ownerControls(item));
      grid.appendChild(card);
    });
    document.querySelector("#gallery-count").textContent =
      `${String(visible.length).padStart(2, "0")} ${current === "videos" ? "VIDEOS" : "ARTWORKS"}`;
    const emptyMessage = document.querySelector("#empty-gallery");
    emptyMessage.textContent =
      current === "videos"
        ? "No videos have been added yet."
        : "No artwork has been added to this collection yet.";
    emptyMessage.hidden = visible.length !== 0;
    const directId = params.get("artwork");
    if (directId && !directArtworkOpened) {
      const directIndex = visible.findIndex((item) => item.id === directId);
      if (directIndex >= 0) {
        directArtworkOpened = true;
        setTimeout(() => openLightbox(directIndex));
      }
    }
  }
  ...
  function ownerControls(item) {
    ...
    controls
      ...
      .addEventListener("click", () => openMediaModal(item.type, item));
    controls
  ...
  const updateOwnerUi = () => {
    ...
    toolbar.querySelector("#owner-status").textContent = owner
      ? `Signed in as ${mediaStore.user().email}. Changes sync across devices.`
      : "Sign in with the authorized Google account.";
    toolbar.querySelector("[data-owner-signin]").hidden = owner;
    toolbar.querySelectorAll("[data-add-media]").forEach((button) => {
      button.hidden = !owner;
    });
    ...
    if (
      owner &&
      customCurrent &&
      !toolbar.querySelector("[data-edit-section]")
    ) {
      const edit = document.createElement("button");
      edit.type = "button";
      edit.dataset.editSection = "";
      edit.textContent = "EDIT SECTION";
      edit.addEventListener("click", () => openSectionModal(customCurrent));
      const remove = document.createElement("button");
      remove.type = "button";
      remove.dataset.deleteSection = "";
      ...
      remove.addEventListener("click", async () => {
        if (
          ...
          location.href = "index.html?owner=1";
        }
      });
      toolbar
        .querySelector("div")
        .insertBefore(remove, toolbar.querySelector("[data-share-site]"));
      toolbar.querySelector("div").insertBefore(edit, remove);
    }
    renderGallery();
  };
  updateOwnerUi();
  ...
  toolbar
    ?.querySelector("[data-owner-signin]")
    ?.addEventListener("click", async () => {
      try {
        await mediaStore.signIn();
      } catch (error) {
        alert(error.message);
      }
    });
  toolbar
    ...
    .forEach((button) =>
      button.addEventListener("click", () =>
        openMediaModal(button.dataset.addMedia),
      ),
    );
  toolbar
    ?.querySelector("[data-share-site]")
    ?.addEventListener("click", () =>
      shareUrl(
        `${location.origin}${location.pathname.replace(/gallery\.html$/, "")}`,
        "Sultan Art Gallery",
      ),
    );
  toolbar
    ?.querySelector("[data-owner-exit]")
    ?.addEventListener("click", async () => {
      await mediaStore.signOut();
  ...
  function openMediaModal(kind, item = null) {
    ...
    document.querySelector("#media-modal-title").textContent = item
      ? `EDIT ${kind.toUpperCase()}`
      : `ADD ${kind === "video" ? "VIDEO" : "ARTWORK"}`;
    document.querySelector("#media-file-label").firstChild.textContent =
    ...
    document.querySelector("#media-collection-label").hidden = kind === "video";
    document.querySelector("#media-featured-label").hidden = kind === "video";
    ...
    mediaStore.sections().forEach((section) => {
      const option = document.createElement("option");
      option.dataset.customOption = section.id;
      option.value = section.slug;
      option.textContent = section.name;
      collectionSelect.appendChild(option);
    });
    collectionSelect.value =
      item?.collection || (current === "videos" ? "my-drawings" : current);
    document.querySelector("#media-featured").checked = Boolean(item?.featured);
    document.querySelector(".media-save").textContent = item
      ? "SAVE CHANGES"
      : `SAVE / ADD ${kind === "video" ? "VIDEO" : "ARTWORK"}`;
    modal.hidden = false;
    document.body.classList.add("viewer-open");
    document.querySelector(".media-modal-close").focus();
  }
  ...
  form.addEventListener("submit", async (event) => {
    ...
    const kind = document.querySelector("#media-kind").value;
    const existing = mediaStore.local().find((item) => item.id === id);
    const file = fileInput.files[0];
    const saveButton = document.querySelector(".media-save");
    ...
    try {
      ...
      if (file) {
        ...
        if (file.size > max)
          throw new Error(
            `File is too large. Maximum ${kind === "video" ? "100" : "20"} MB.`,
          );
      ...
      const item = {
        ...
        description: document.querySelector("#media-description").value.trim(),
        year: document.querySelector("#media-year").value.trim(),
        collection:
          kind === "video"
            ? "videos"
            : document.querySelector("#media-collection").value,
        url: upload?.secure_url || existing?.url,
        thumbnail: "",
        publicId: upload?.public_id || existing?.publicId || "",
        featured:
          kind === "image" && document.querySelector("#media-featured").checked,
        width: upload?.width || existing?.width || 0,
        height: upload?.height || existing?.height || 0,
        orientation: orientationFromDimensions(
          upload?.width || existing?.width,
          upload?.height || existing?.height,
        ),
        order: existing?.order ?? mediaStore.local().length,
  ...
  async function shareUrl(url, title) {
    try {
      ...
      if (error.name !== "AbortError") alert("Could not share this link.");
    }
  }
  ...
  function previewBackground() {
    const type = document.querySelector("#background-type").value,
      value = document.querySelector("#background-value").value.trim(),
      preview = document.querySelector("#background-preview");
    preview.style.background =
      type === "image" && value
        ? `url("${value.replace(/["'()]/g, "")}") center/cover`
        : type === "gradient"
          ? value || "linear-gradient(120deg,#07070d,#421378)"
          : type === "color"
            ? value || "#07070d"
            : "var(--bg)";
  }
  function openSettings() {
    ...
    if (!document.querySelector("#background-file")) {
      ...
      uploadLabel.textContent = "UPLOAD BACKGROUND IMAGE ";
      const uploadInput = document.createElement("input");
      uploadInput.id = "background-file";
      uploadInput.type = "file";
      uploadInput.accept = "image/jpeg,image/png,image/webp,image/gif";
      uploadInput.addEventListener("change", () => {
        const file = uploadInput.files[0];
        if (file)
          document.querySelector("#background-preview").style.background =
            `url("${URL.createObjectURL(file)}") center/cover`;
      });
      uploadLabel.appendChild(uploadInput);
      document.querySelector("#background-preview").before(uploadLabel);
    }
    document.querySelector("#background-type").value = value.backgroundType;
    document.querySelector("#background-value").value = value.backgroundValue;
    document.querySelector("#setting-brand").value = value.brand;
    document.querySelector("#setting-gallery").value = value.gallery;
    document.querySelector("#setting-eyebrow").value = value.eyebrow;
    document.querySelector("#setting-statement").value = value.statement;
    document.querySelector("#setting-font").value = value.font;
    document.querySelector("#setting-font-size").value = value.fontSize;
    document.querySelector("#setting-site-title").value = value.siteTitle;
    document.querySelector("#setting-site-description").value =
      value.siteDescription;
    document.querySelector("#setting-social-image").value = value.socialImage;
    document.querySelector("#setting-home-video").value = value.homeVideo || "";
  ...
  function renderSectionManagement() {
    ...
    if (!panel) {
      ...
      panel.id = "section-management";
      panel.className = "section-management";
    ...
    sections.forEach((section, index) => {
      ...
      const moveSection = async (step) => {
        const ids = sections.map((value) => value.id);
        const target = index + step;
        [ids[index], ids[target]] = [ids[target], ids[index]];
        await mediaStore.reorderSections(ids);
      };
      row
        .querySelector("[data-up]")
        .addEventListener("click", () => moveSection(-1));
      row
        .querySelector("[data-down]")
        .addEventListener("click", () => moveSection(1));
  ...
  document
    .querySelector("#background-type")
    ?.addEventListener("change", previewBackground);
  document
    .querySelector("#background-value")
    ?.addEventListener("input", previewBackground);
  settingsForm?.addEventListener("submit", async (event) => {
    ...
    try {
      const backgroundFile =
        document.querySelector("#background-file")?.files[0];
      if (backgroundFile) {
        if (backgroundFile.size > 20 * 1024 * 1024)
          throw new Error("Background image must be 20 MB or smaller.");
        const upload = await mediaStore.upload(
          backgroundFile,
          "image",
          () => {},
        );
        backgroundUploadUrl = upload.secure_url;
        document.querySelector("#background-type").value = "image";
      }
      await mediaStore.saveSettings({
        backgroundType: document.querySelector("#background-type").value,
        backgroundValue:
          backgroundUploadUrl ||
          document.querySelector("#background-value").value.trim(),
        brand:
          document.querySelector("#setting-brand").value.trim() ||
          defaultSettings.brand,
        gallery:
          document.querySelector("#setting-gallery").value.trim() ||
          defaultSettings.gallery,
        eyebrow: document.querySelector("#setting-eyebrow").value.trim(),
        statement: document.querySelector("#setting-statement").value.trim(),
        font: document.querySelector("#setting-font").value,
        fontSize: Number(document.querySelector("#setting-font-size").value),
        siteTitle:
          document.querySelector("#setting-site-title").value.trim() ||
          defaultSettings.siteTitle,
        siteDescription: document
          .querySelector("#setting-site-description")
          .value.trim(),
        socialImage: document
          .querySelector("#setting-social-image")
          .value.trim(),
          homeVideo: document.querySelector("#setting-home-video").value.trim(),
      });
  ...
  document
    ...
    ?.addEventListener("click", async () => {
      await mediaStore.saveSettings(defaultSettings);
      openSettings();
    });
  ...
  sectionForm?.addEventListener("submit", async (event) => {
    ...
    const name = document.querySelector("#section-name").value.trim(),
      id = document.querySelector("#section-id").value,
      base =
        name
          .toLowerCase()
          .normalize("NFKD")
    ...
    try {
      ...
      document.querySelector("#section-status").textContent = error.message;
    }
  });
})();
